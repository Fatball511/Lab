...
Dm = np.mean(WD)#mean of WD
s1 = np.std(WD) #Sd of WD
WO = H.trestbps.loc[H.hasHeartDisease==0]#resting blood pressure without disease
Nm = np.mean(WO) #mean of WO
s2 = np.std(WO) #Sd of WO
n1,n2 = len(WD),len(WO)
...
print(p)
z_score = 1.96
p_fm = 24/96 #population proportion of female 
n_f = 96 #sample size of female
p_m = 114/207 #population proportion of male
n_m = 207 #sample size of male
pp = p_m - p_fm 
...
se_diff = sqrt(se_female**2 + se_male**2)#Standard Error Difference between female & male
